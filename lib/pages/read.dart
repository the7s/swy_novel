...
class _ReadPageState extends State<ReadPage> {
  ...
  _fetchDetail(String url, {Function post}) async {
    ...
    try {
      ...
      // print(detailResult);
      var result = await getNovelDetail(url);
  ...
  _fetchChapterList(String chapterUrl) async {
    try {
      ...
      var result = await getNovelChapterList(url);    
      setState(() {
        _chapterList = result;
      });
    } catch (e) {
  ...
  _initData() async {
    ...
    double currPos =
        prefs.getDouble(widget.novelId.toString() + 'currPos') ?? 0.0;
  ...
  _saveData() async {
    ...
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setDouble(widget.novelId.toString() + 'currPos', _currPos);
    String shelfListStr = prefs.getString('shelfList');
    List shelfList = [];
    List data = jsonDecode(shelfListStr);
    data.forEach((item) {
      if(item['novelId'] == widget.novelId){
        item['recentChapterUrl'] =_current_url;
      }
      shelfList.add(item);
    });
